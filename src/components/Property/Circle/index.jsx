...
class Circle extends Component {
  ...
  handleProcess = async() => {
    if(this.props.isProcessing){
      ...
      let re = await window.grpc.processCircle({...this.props.currentObj.userData.data, isLast:true})
  ...
  render() {
    ...
    return (
      <Space direction='vertical' >
        <Space>
          圆心：
          <InputNumber value={data.X1} onChange={this.handleChange('X1')} controls={false} addonBefore='X' size='small' />
          <InputNumber value={data.Y1} onChange={this.handleChange('Y1')} controls={false} addonBefore='Y' size='small' />
        </Space>
        
        <Space>
          起点：
          <InputNumber value={data.X2} onChange={this.handleChange('X2')} controls={false} addonBefore='X' size='small' />
          <InputNumber value={data.Y2} onChange={this.handleChange('Y2')} controls={false} addonBefore='Y' size='small' />
        </Space>
        <Space>
          <Radio.Group value={data.M} onChange={this.handleRadio('M')}>
            <Space>
        ...
        <Space>
          <InputNumber value={data.angle} min={0} max={360} onChange={this.handleChange('angle')} controls={false} addonBefore='圆弧角度' addonAfter='°' size='small' />
          <InputNumber value={data.taper} onChange={this.handleChange('taper')} controls={360} addonBefore='锥度' size='small' />
        </Space>
        ...
        <Space style={{'display':data.filled ? 'block':'none'}}>
          <Card size='small' title='半径' headStyle={{textAlign:'left'}}>
            <Space direction='vertical'>
              <InputNumber value={userData.getRadius()} disabled controls={false} addonBefore='外圆半径R' size='small' />
              <InputNumber value={data.r_min} max={userData.getRadius()} onChange={this.handleChange('r_min')} controls={false} addonBefore='内圆半径r ' size='small' />
              <InputNumber value={data.r_interval} onChange={this.handleChange('r_interval')} controls={false} addonBefore='填充间隔  ' size='small' />
            </Space>
          </Card>
